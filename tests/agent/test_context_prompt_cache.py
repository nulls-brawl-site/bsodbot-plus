...
def test_recent_history_capped_at_max(tmp_path) -> None:
    ...
    for i in range(builder._MAX_RECENT_HISTORY + 20):
        builder.memory.append_history(f"entry-{i:04d}")

    prompt = builder.build_system_prompt()
    assert "entry-0000" not in prompt
    assert "entry-0019" not in prompt
    assert f"entry-{builder._MAX_RECENT_HISTORY + 19:04d}" in prompt
...
def test_recent_history_truncated_at_max_chars(tmp_path) -> None:
    ...
    assert len(history_section[1]) < builder._MAX_HISTORY_CHARS + 500


def test_recent_history_keeps_processed_archive_entries(tmp_path) -> None:
    """Dream-processed archive entries still appear for conversational recall."""
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)
    ...
    prompt = builder.build_system_prompt()
    assert "# Recent History" in prompt
    assert "already processed entry" in prompt


def test_partial_dream_processing_shows_only_remainder(tmp_path) -> None:
    """The archive section includes processed and unprocessed recent entries."""
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)
    ...
    prompt = builder.build_system_prompt()
    assert "# Recent History" in prompt
    assert "old conversation about Python" in prompt
    assert "old conversation about Rust" in prompt
