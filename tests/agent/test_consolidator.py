...
class TestConsolidatorSummarize:
    ...
    async def test_archive_input_preserves_tool_calls_and_tool_names(
        self,
        consolidator,
        mock_provider,
    ):
        """Compaction input must retain what tools/code actions happened."""
        mock_provider.chat_with_retry.return_value = MagicMock(
            content="Used exec to run pytest.", finish_reason="stop"
        )
        messages = [
                ...
                "tool_calls": [
                    {
                        ...
                        "function": {
                            "name": "exec",
                            "arguments": '{"command":"pytest tests/agent/test_context_prompt_cache.py"}',
                        },
                    }
                ],
            },
        ...
        sent = mock_provider.chat_with_retry.call_args.kwargs["messages"][1]["content"]
        assert "tool calls: exec#call-1" in sent
        assert "pytest tests/agent/test_context_prompt_cache.py" in sent
        assert "TOOL [name: exec; id: call-1]: 1 passed" in sent
