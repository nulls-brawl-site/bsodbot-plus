...
def test_streaming_apply_patch_tracker_counts_line_edits(tmp_path: Path) -> None:
    ...
    async def run() -> None:
        ...
        await tracker.update({
            "index": 0,
            "call_id": "call-patch-lines",
            "name": "apply_patch",
            "arguments_delta": (
                '{"edits":[{"path":"src/existing.py","action":"replace_lines",'
                '"start_line":2,"end_line":3,"new_text":"TWO"}]}'
            ),
        })
    ...
    assert len(events) == 1
    assert events[0]["path"] == "src/existing.py"
    assert (events[0]["added"], events[0]["deleted"]) == (1, 2)
