...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    assert any(cmd.command == "plan" for cmd in app.bot.commands)
    assert any(cmd.command == "plan_on" for cmd in app.bot.commands)
    assert any(cmd.command == "plan_off" for cmd in app.bot.commands)
    assert any(cmd.command == "plan_clear" for cmd in app.bot.commands)
...
@pytest.mark.asyncio
async def test_forward_command_normalizes_telegram_safe_plan_aliases() -> None:
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["*"], group_policy="open"),
        MessageBus(),
    )
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/plan_off@nanobot_test", reply_to_message=None)

    await channel._forward_command(update, None)

    assert len(handled) == 1
    assert handled[0]["content"] == "/plan off"


def test_normalize_telegram_plan_aliases() -> None:
    assert TelegramChannel._normalize_telegram_command("/plan_on") == "/plan on"
    assert TelegramChannel._normalize_telegram_command("/plan_off") == "/plan off"
    assert TelegramChannel._normalize_telegram_command("/plan_clear") == "/plan clear"
    assert TelegramChannel._normalize_telegram_command("/plan_on fix context") == "/plan on fix context"
...
def test_telegram_bus_slash_command_regex_matches_agent_loop_commands() -> None:
    ...
    assert pat.fullmatch("/plan")
    assert pat.fullmatch("/plan on")
    assert pat.fullmatch("/plan_on")
    assert pat.fullmatch("/plan_off@nanobot_bot")
    assert pat.fullmatch("/plan_clear")
...
def test_build_keyboard_uses_button_values_for_callback_data() -> None:
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", inline_keyboards=True),
        MessageBus(),
    )

    markup = channel._build_keyboard([["Gemini Pro"]], [["/model gemini-3.5-pro"]])
    btn = markup.inline_keyboard[0][0]

    assert btn.text == "Gemini Pro"
    assert btn.callback_data == "/model gemini-3.5-pro"
