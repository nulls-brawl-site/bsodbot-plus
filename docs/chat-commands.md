...
| `/models` | Browse model providers and choose a model with inline buttons |
| `/plan` | Show the current executable plan |
| `/plan on` | Enable plan mode for the session |
| `/plan <objective>` | Ask the agent to create/update a plan and work through it |
...
The response shows the current model, active preset, available preset names, and each preset's model/provider/output/context settings. `default` is always available and represents the model settings from `agents.defaults.*`.
...
Preset names come from the top-level `modelPresets` config. Switching is runtime-only: it does not rewrite `config.json`, and an in-progress turn keeps using the model it started with. Use `/models` for an interactive provider/model picker. See [Configuration: Model presets](./configuration.md#model-presets) for setup details.

## Executable Plans

Use `/plan <objective>` when you want nanobot to keep a visible working plan and execute it across turns:

```text
/plan refactor the Telegram channel tests and run the relevant suite
```

The agent records the plan with `update_plan`, marks progress as steps move from `pending` to `in_progress` to `completed`, and keeps the plan in runtime context so compaction does not hide it. This is not read-only planning; the agent should keep working unless you explicitly ask for a plan only.
