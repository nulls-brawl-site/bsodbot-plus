...
nanobot replays recent session messages directly before falling back to compact archives. By default it keeps up to 500 live messages in the model prompt, subject to the configured token budget.
...
| Option | Default | Description |
|--------|---------|-------------|
| `agents.defaults.maxMessages` | `500` | Maximum live session messages to replay before older turns are archived. Set to `0` to use the built-in default. Token limits still apply. |

When older turns must be compacted, the archive now preserves user requests, tool calls, commands, file/code changes, test runs, errors, and decisions so future turns can still recover what happened.
...
>
> Concretely, auto compact rewrites `sessions/<key>.jsonl` in place: older messages (including their structured `tool_calls` / `tool_call_id` / `reasoning_content`) are replaced by just the retained recent suffix (currently 8 messages), while the archived prefix is preserved as a tool/code-aware plain-text archive appended to `memory/history.jsonl` (or a `[RAW] ...` flattened dump if LLM summarization fails). The original structured JSON of those turns is no longer recoverable from the session file.
>
