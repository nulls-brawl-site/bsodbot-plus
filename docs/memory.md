...
When a conversation grows large enough to pressure the context window, nanobot first keeps a much larger live replay window (500 messages by default) and only compacts the older prefix when token limits or replay limits require it.

The `Consolidator` summarizes the oldest safe slice of the conversation and appends that summary to `memory/history.jsonl`. These summaries preserve operational context: user requests, tool calls, commands, file/code edits, tests, errors, decisions, and current status.
