...
class AgentDefaults(Base):
    ...
    max_messages: int = Field(
        default=500,
        ge=0,
    )  # Max messages to replay from session history (0 = use default 500, respects token budget)
...
class ProviderConfig(Base):
    ...
    proxy: str | None = None  # Provider-specific HTTP(S) proxy URL; avoids global env proxy leaks
...
class ProvidersConfig(Base):
    ...
    closerouter: ProviderConfig = Field(default_factory=ProviderConfig)  # CloseRouter OpenAI-compatible gateway
    bluesminds: ProviderConfig = Field(default_factory=ProviderConfig)  # BluesMinds OpenAI-compatible gateway
    ...
    def _validate_api_type_scope(self) -> "ProvidersConfig":
        # api_type selects which OpenAI-compatible API surface to use
        # (auto/chat/responses).  Custom and third-party gateways may expose
        # only one of these surfaces, so this setting is no longer limited to
...
class Config(BaseSettings):
    ...
    def _normalize_provider_aliases(cls, data: Any) -> Any:
        """Accept historical/free-form provider aliases in config files.

        The schema uses snake_case field names internally, while config files
        commonly use camelCase.  Some provider IDs are user-facing slugs (for
        example "free-gpt") that are not valid Python identifiers, so normalize
        those aliases before pydantic validates the ProvidersConfig model.
        """
        if isinstance(data, dict):
            providers = data.get("providers")
            if isinstance(providers, dict):
                if "freeGpt" in providers and "custom" in providers:
                    # Historical quick patch: freeGpt mirrors custom.  Fold the
                    # explicit apiType/key/base into custom so free-gpt presets
                    # don't silently lose provider settings.
                    merged = dict(providers.get("custom") or {})
                    merged.update(providers.get("freeGpt") or {})
                    providers["custom"] = merged
                alias_map = {
                    "freeGpt": "custom",
                    "free-gpt": "custom",
                    "free_gpt": "custom",
                    "blueMinds": "bluesminds",
                    "blueminds": "bluesminds",
                    "blue_minds": "bluesminds",
                }
                for alias, canonical in alias_map.items():
                    if alias in providers and canonical not in providers:
                        providers[canonical] = providers[alias]
        return data
