...
PROVIDERS: tuple[ProviderSpec, ...] = (
    ...
    # CloseRouter: OpenAI-compatible gateway, keys start with "closerouter_"
    ProviderSpec(
        name="closerouter",
        keywords=("closerouter",),
        env_key="CLOSEROUTER_API_KEY",
        display_name="CloseRouter",
        backend="openai_compat",
        is_gateway=True,
        detect_by_key_prefix="closerouter_",
        detect_by_base_keyword="closerouter",
        default_api_base="https://api.closerouter.dev/v1",
    ),
    # BluesMinds: OpenAI-compatible gateway.
    ProviderSpec(
        name="bluesminds",
        keywords=("bluesminds", "blueminds"),
        env_key="BLUESMINDS_API_KEY",
        display_name="BluesMinds",
        backend="openai_compat",
        is_gateway=True,
        detect_by_base_keyword="bluesminds",
        default_api_base="https://api.bluesminds.com/v1",
    ),
...
def find_by_name(name: str) -> ProviderSpec | None:
    """Find a provider spec by config field name, accepting known aliases."""
    normalized = to_snake(name.replace("-", "_"))
    normalized = _PROVIDER_NAME_ALIASES.get(normalized, normalized)
