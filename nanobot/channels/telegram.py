...
class TelegramChannel(BaseChannel):
    ...
    BOT_COMMANDS = [
        ...
        BotCommand("model", "Show or switch runtime model preset"),
        BotCommand("models", "Browse model providers"),
        BotCommand("plan", "Show plan status or create a plan"),
        BotCommand("plan_on", "Enable executable plan mode"),
        BotCommand("plan_off", "Disable executable plan mode"),
        BotCommand("plan_clear", "Clear current plan"),
    ...
    TELEGRAM_BUS_SLASH_COMMAND_RE = re.compile(
        r"^/(?:new|stop|restart|status|dream|history|goal|pairing|model|models|plan|plan_on|plan_off|plan_clear)(?:@\w+)?(?:\s+.*)?$"
    )
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if msg.content and msg.content != "[empty message]":
            ...
            button_values = getattr(msg, "button_values", None) or []
            reply_markup = self._build_keyboard(buttons, button_values) if buttons else None
            text = msg.content
    ...
    def _build_keyboard(
        self,
        buttons: list,
        button_values: list | None = None,
    ) -> InlineKeyboardMarkup | None:
        """Build inline keyboard markup if inline_keyboards is enabled."""
        if not buttons or not self.config.inline_keyboards:
            return None
        values = button_values or []
        keyboard = []
        for row_index, row in enumerate(buttons):
            value_row = values[row_index] if row_index < len(values) and isinstance(values[row_index], list) else []
            keyboard.append([
                InlineKeyboardButton(
                    label,
                    callback_data=self._safe_callback_data(
                        value_row[col_index]
                        if col_index < len(value_row) and isinstance(value_row[col_index], str)
                        else label
                    ),
                )
