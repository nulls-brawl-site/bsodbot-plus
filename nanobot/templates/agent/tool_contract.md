...
- For surgical edits after `read_file`, prefer `apply_patch` line actions: `replace_lines`, `insert_lines`, or `delete_lines` with 1-based `start_line`/`end_line` and optional `expected_old_text` guard.
