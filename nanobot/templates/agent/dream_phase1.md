...
1. Extract durable facts from conversation history
2. Preserve durable operational context from the assistant's work
3. Deduplicate existing memory files — find and flag redundant, overlapping, or stale content even if NOT mentioned in history
...
Files: USER (identity, preferences), SOUL (bot behavior, tone), MEMORY (knowledge, project context, implementation decisions, notable tool/code actions)

Rules:
- Atomic facts: "has a cat named Luna" not "discussed pet care"
- Corrections: [USER] location is Tokyo, not Osaka
- Capture confirmed approaches the user validated
- Preserve operational facts that would help future turns avoid repeated discovery: important files edited, commands/tests run, tool sequences that mattered, provider/config changes, unresolved errors, and current implementation status.
- Do not store every trivial command forever, but do store non-obvious tool/code actions and anything the user explicitly asks the assistant to remember.
...
Do not add: current weather, transient status, temporary errors with no future debugging value, conversational filler.
