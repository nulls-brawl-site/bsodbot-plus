...
API_CHAT_ID = "default"
APP_AGENT_LOOP = web.AppKey("agent_loop", Any)
APP_MODEL_NAME = web.AppKey("model_name", str)
APP_REQUEST_TIMEOUT = web.AppKey("request_timeout", float)
APP_SESSION_LOCKS = web.AppKey("session_locks", dict)


def _app_get(app: Any, key: web.AppKey, legacy_key: str, default: Any = None) -> Any:
    """Read aiohttp AppKey values while keeping direct handler unit tests simple."""
    try:
        return app[key]
    except KeyError:
        if hasattr(app, "get"):
...
async def handle_chat_completions(request: web.Request) -> web.Response:
    ...
    agent_loop = _app_get(request.app, APP_AGENT_LOOP, "agent_loop")
    timeout_s: float = _app_get(request.app, APP_REQUEST_TIMEOUT, "request_timeout", 120.0)
    model_name: str = _app_get(request.app, APP_MODEL_NAME, "model_name", "nanobot")
    ...
    session_locks: dict[str, asyncio.Lock] = _app_get(
        request.app, APP_SESSION_LOCKS, "session_locks", {}
    )
...
async def handle_models(request: web.Request) -> web.Response:
    """GET /v1/models"""
    model_name = _app_get(request.app, APP_MODEL_NAME, "model_name", "nanobot")
    return web.json_response(
        {
            "object": "list",
            "data": [
...
def create_app(
    ...
    app[APP_AGENT_LOOP] = agent_loop
    app[APP_MODEL_NAME] = model_name
    app[APP_REQUEST_TIMEOUT] = request_timeout
    app[APP_SESSION_LOCKS] = {}  # per-user locks, keyed by session_key
