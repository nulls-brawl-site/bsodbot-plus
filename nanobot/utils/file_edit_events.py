    ...
    async def _update_apply_patch(self, state: _StreamingFileEditState) -> None:
        ...
        for i, m in enumerate(path_matches):
            ...
            action_match = re.search(
                r'"action"\s*:\s*"(replace|add|delete|replace_lines|insert_lines|delete_lines)"',
                segment,
            )
            action = action_match.group(1) if action_match else "replace"

            old_text = _extract_json_string_prefix(segment, "old_text") or ""
            expected_old_text = _extract_json_string_prefix(segment, "expected_old_text") or ""
            new_text = _extract_json_string_prefix(segment, "new_text") or ""

            if action == "insert_lines":
                added = _text_line_count(new_text)
                deleted = 0
            elif action == "replace_lines":
                added = _text_line_count(new_text)
                deleted = _line_range_count(segment) or _text_line_count(expected_old_text or old_text)
...
def _extract_json_int_prefix(source: str, key: str) -> int | None:
    match = re.search(rf'"{re.escape(key)}"\s*:\s*(\d+)\b', source)
    if match is None:
        return None
    try:
...
def _line_range_count(source: str) -> int:
    start = _extract_json_int_prefix(source, "start_line")
    if start is None:
        return 0
    end = _extract_json_int_prefix(source, "end_line") or start
    if end < start:
        return 0
    return end - start + 1
