...
async def _probe_http_url(url: str, timeout: float = 3.0) -> bool:
    ...
    try:
        ...
        with suppress(OSError, asyncio.TimeoutError):
            await asyncio.wait_for(writer.wait_closed(), timeout=min(timeout, 0.5))
        return True
    ...
    async def connect_single_server(name: str, cfg) -> tuple[str, AsyncExitStack | None]:
        ...
        try:
            ...
            if any(
                ...
                hint = (
                    " Hint: this looks like stdio protocol pollution. Make sure the MCP "
                    "server writes only JSON-RPC to stdout and sends logs/debug output "
                    "to stderr instead."
                )
...
async def reload_servers(state: Any, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        ...
        logger.info(
            "MCP hot reload: added={} changed={} removed={} retried={} connected={} "
