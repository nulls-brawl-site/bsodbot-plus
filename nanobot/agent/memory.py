...
class MemoryStore:
    ...
    _TOOL_CALL_ARGS_MAX_CHARS = 1_200
    ...
    def read_recent_history(self, limit: int) -> list[dict[str, Any]]:
        """Return the most recent valid history entries, including Dream-processed ones."""
        if limit <= 0:
            return []
        entries = [e for e, _ in self._iter_valid_entries()]
    ...
    @classmethod
    def _format_tool_calls(cls, tool_calls: Any) -> str:
        ...
        for call in tool_calls:
            ...
            args_text = truncate_text(str(args), cls._TOOL_CALL_ARGS_MAX_CHARS) if args else ""
            call_id = call.get("id") or call.get("tool_call_id")
            call_label = f"{name}#{call_id}" if call_id else name
            formatted.append(f"{call_label}({args_text})" if args_text else call_label)
        return "; ".join(formatted)

    @classmethod
    def _format_messages(cls, messages: list[dict]) -> str:
        lines = []
        for message in messages:
            content = cls._stringify_message_content(message.get("content"))
            tool_calls = cls._format_tool_calls(message.get("tool_calls"))
            if not content and not tool_calls:
                continue
            label = str(message.get("role", "?")).upper()
            meta: list[str] = []
            if message.get("tools_used"):
                meta.append(f"tools used: {', '.join(message['tools_used'])}")
            ...
            lines.append(
                f"[{message.get('timestamp', '?')[:16]}] {label}{suffix}: {content or '(no text)'}"
            )
        return "\n".join(lines)
